...
const Signin = () => {
    ...
    return (
        <div className="body-bg bg-purple-600 h-full inset-y-0 pt-12 md:pt-20 pb-6 px-2 md:px-0 relative" >
           { auth.error && <Error title="Login Failed" body={auth.error} action={auth.closeError} />}
            ...
            <header className="max-w-lg mx-auto">
                <a href="#">
                    <h1 className="text-4xl font-bold text-white text-center">T-Shop</h1>
                </a>
            </header>
                ...
                <section className="mt-10">
                    <form className="flex flex-col" method="POST" action="#">
                        <div className="mb-6 pt-3 rounded bg-gray-200">
                            ...
                            <input onChange={(e) => setEmail(e.target.value)} type="text" id="email" className="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-purple-600 transition duration-500 px-3 pb-3" />
                        </div>
                        <div className="mb-6 pt-3 rounded bg-gray-200">
                        ...
                        <button onClick={signIn} className={` text-white font-bold py-2 rounded shadow-lg hover:shadow-xl transition duration-200 ${loading ? 'bg-purple-400 cursor-wait' : 'bg-purple-600 hover:bg-purple-700'}`} type="submit">{loading ? "loading..." : "Sign In"}</button>
                    </form>
                </section>
            </main>

            <div className="max-w-lg mx-auto text-center mt-12 mb-6">
                <Link to="/signup" className="text-white">Don't have an account? <p href="#" className="font-bold hover:underline">Sign up</p>.</Link>
            </div>
